...
* **Миграции:** В отличие от некоторых других модулей, в `task` включен **Liquibase** (`enabled: true`), что позволяет автоматически обновлять структуру таблиц при деплое через файл `db.changelog-client.xml`
...
actor "Супервизор" as User #cce5ff
participant "Data Import Service" as DataSvc #d4edda

box "HikariCP Pool (Size: 10)" #ffffff
    participant "Conn Pool" as Pool #fff3cd
    participant "Connection 1..10" as Conn #f8d7da
end box

database "PostgreSQL" as DB #e2e3e5
participant "MinIO / Local FS" as Storage #f0f0f0
...
DataSvc -> DB : [Schema: import]\nINSERT INTO import_batches (Статус: In Progress)
DB -->> DataSvc : ACK
...
DataSvc -> DB : [Schema: import]\nUPDATE import_batches (Статус: Success)
DB -->> DataSvc : ACK
